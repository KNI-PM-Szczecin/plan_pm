...
### Flaga `[skip deploy]`
Jeśli commit message zawiera `[skip deploy]`, oba joby są pomijane. Używaj przy pushach technicznych (synchronizacja gałęzi, hotfixy konfiguracji) które nie powinny trafić na store.
...
### Job iOS → TestFlight
1. Checkout + setup Xcode `latest-stable` (wymagane przez Apple od 28 kwietnia 2026)
2. Flutter pub get
