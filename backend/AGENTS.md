...
Run the full schedule pipeline with `python main.py`; use `python main.py --workers 20` for higher parallelism or `python main.py --old-scrapper` for the Selenium fallback. Start the admin UI with `python admin/app.py` and open `http://localhost:5050`. Run individual stages with module commands such as `python -m json2db.json2db --input ./output/parser.json --dry-run` or `python -m structure_updater.structure_updater --source web --dry-run`.
...
Use the existing `slow` marker for integration tests that require network, Selenium, or external services. Name new tests `test_<module>.py` and keep fixtures close to the module unless they are shared broadly.
