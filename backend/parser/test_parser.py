...
def test_tokStringToDic_collapses_doubled_space_in_name():
    """
    Plan 'Inżynieria i Bezpieczeństwo  w Transporcie Drogowym' przyjeżdża ze
    strony z podwójną spacją, a struktura (combobox) z pojedynczą. Aplikacja
    matchuje nazwy przez .eq(), więc bez normalizacji cała specjalizacja jest
    nieosiągalna z poziomu apki.
    """
    result = Parser.tokStringToDic(
        "Inżynieria i Bezpieczeństwo  w Transporcie Drogowym S inż. 3.50 2023/2024 zima"
    )
    assert result["name"] == "Inżynieria i Bezpieczeństwo w Transporcie Drogowym"
    assert result["program_type"] == "S"
    assert result["degree_level"] == "inż."
    assert result["academic_year"] == "2023/2024 zima"


def test_tokStringToDic_collapses_nbsp_in_name():
    result = Parser.tokStringToDic(
        "Transport\xa0 Morski S inż. 4.00 2024/2025 zima"
    )
    assert result["name"] == "Transport Morski"


def test_tokStringToDic_name_with_language_suffix_is_normalized():
    """'Mechanika i Budowa Maszyn  ANG' — podwójna spacja przed znacznikiem języka.

    Znacznik zostaje (patrz test niżej), ale spacje muszą się zwinąć, inaczej
    aplikacja nie dopasuje nazwy do struktury.
    """
    result = Parser.tokStringToDic(
        "Mechanika i Budowa Maszyn  ANG S inż. 4.00 2024/2025 zima"
    )
    assert result["name"] == "Mechanika i Budowa Maszyn ANG"
    assert result["language"] == "ANG"


def test_tokStringToDic_unknown_degree_level_name_is_normalized():
    result = Parser.tokStringToDic("Kurs  bez stopnia 3.50 2024/2025 zima")
    assert result["name"] == "Kurs bez stopnia 3.50 2024/2025 zima"
    assert result["degree_level"] == ""


def test_tokStringToDic_keeps_language_marker_in_name():
    """
    Znacznik języka musi zostać w nazwie — v_unique_groups nie ma kolumny
    language, więc to jedyne, po czym aplikacja odróżni tok angielskojęzyczny
    od polskiego. Wcześniej "ANG" było obcinane, a "ang." nie.
    """
    lower = Parser.tokStringToDic("Transport Morski ang. S inż. 4.00 2026/2027 zima")
    upper = Parser.tokStringToDic("Transport Morski ANG S inż. 4.00 2025/2026 zima")
    assert lower["name"] == "Transport Morski ang."
    assert lower["language"] == "ANG"
    assert upper["name"] == "Transport Morski ANG"
    assert upper["language"] == "ANG"


def test_tokStringToDic_polish_is_default_language():
    result = Parser.tokStringToDic("Nawigacja S inż. 4.00 2026/2027 zima")
    assert result["name"] == "Nawigacja"
    assert result["language"] == "POL"


def test_normalize_name_is_idempotent():
    from parser.parser import normalize_name

    once = normalize_name("  Logistyka   Transportu  Zintegrowanego ")
    assert once == "Logistyka Transportu Zintegrowanego"
