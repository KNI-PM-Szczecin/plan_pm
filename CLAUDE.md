...
- **Jeden skalowalny (resizable) widżet** — pojedyncza klasa `ScheduleWidgetProvider` w [`ScheduleWidgetProvider.kt`](frontend/android/app/src/main/kotlin/com/piotrwittig/plan_pm/ScheduleWidgetProvider.kt) (iOS ma osobne, stałe rozmiary — to dotyczy tylko Androida):
  ...
  - **Sala na wąskim widżecie (kompaktowanie zamiast chowania):** wiersz czas+sala jest mierzony (`timePaint`, 11sp) i dopasowywany stopniowo — pełny czas + pełna sala → pełny czas + sam numer sali (`shortRoom`, „WChrobrego 208" → „208") → sam początek godziny + numer sali. Sala znika tylko gdy zajęcia jej nie mają. Pole sali ma `maxLines=1`+`ellipsize=end` jako ostateczny fallback.
